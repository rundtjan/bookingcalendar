...
var rooms = [
  ...
  time = [
    ...
    [11, 12.5, "11.00"],
    [12.5, 14, "12.30-14.00"],
    [14.25, 15.75, "14.15-15.45"],
    [16.5, 18, "16.30-18.00"],
    [18.083, 19.583, "18.05-19.35"],
    [19.75, 21.25, "19.45-21.15"],
  ],
  lessons = time.length,
  marginUp = 10,
  roomWidth = 160,
...
function ajaxRequest(method, url, callback, sendString) {
  //standard ajax-function
  console.log("sending", method, url);
  var xmlhttp = new XMLHttpRequest();

  xmlhttp.onreadystatechange = function () {
    // check if readyState is 4, then execute callback on response
...
topMenu.innerHTML =
  '<div id="logo"><b>Lakanet 2.0</b> - läsåret 2021-2022</div><button type="button" id="showLastYear" onclick="showLastYear()">' +
  buttonText +
  '</button><button type="button" id="logout" onclick="logout()">Logga ut</button>';
...
function logout() {
  ...
  if (
    ...
    window.localStorage.getItem("lakanetSet") == "true"
  ) {
...
for (var z = 0; z < 2; z++) {
  ...
  var x = 100;
  for (var i = 0; i < rooms.length; i++) {
    //adding the rooms for saturdays
...
function addTopRowNames(id) {
  for (var x = 0; x < roomCords.length; x++) {
    //adding names of rooms (top-row)
    var g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    document.getElementById(id).appendChild(g);
    g.setAttribute(
      "transform",
      "translate(" + parseInt(roomCords[x][1]) + ", 0)"
    );
...
function yToTime(y) {
  //finding what time the user has pointed on using the y-coordinate
...
function yToSatTime(y, semester) {
  //finding what time the user has pointed on using the y-coordinate
  if (semester == "ht") {
    eventSaturday = satHT[Math.floor(y / lessonHeight) - 1];
  } else {
...
function timeToY(t, start) {
  //start = true or false = start or end - finding the y-coordinate based on the clocktime
  var check = false;
  var y = marginUp;
  for (var i = 0; i < time.length; i++) {
    if (t >= time[i][0] && t <= time[i][1]) {
      y +=
        i * lessonHeight +
        ((t - time[i][0]) / (time[i][1] - time[i][0])) * lessonHeight;
      check = true;
      i = time.length;
    }
  }
...
function satTimeToY(day) {
  ...
  if (satHT.indexOf(day) > -1) {
    ...
    y += lessonHeight * satVT.indexOf(day);
  }
  return y;
}

function createTimeSh(x, i, id, y) {
  //module for creating the text for the timetable
  if (i == 1) {
    var textDay = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "text"
    );
...
function satCreateTimeSh(x, i, id, y) {
  ...
  text.setAttribute("y", y + 15);
  if (id == "ht") {
    text.innerHTML = "Lör " + satHT[i];
  } else {
    text.innerHTML = "Lör " + satVT[i];
  }
}

function addLines(id, saturday) {
  //creating the lines for the timetable
  var length;
  if (!saturday) {
    length = time.length;
  } else {
    length = satHT.length + 1;
  }
...
function addRect(s, info, x, y, idP, idR, w, h, f, o) {
  //module for creating different rectangles
  if (s == "day") {
    (y = marginUp), (w = roomWidth), (h = roomHeight), (f = "white");
  } else if (s == "saturday") {
    (y = marginUp), (w = roomWidth), (h = satRoomHeight), (f = "white");
  } else if (s == "lesson") {
    (w = roomWidth), (h = lessonHeight), (f = "#d1f3f3");
  } else if (s == "clicker") {
    (y = marginUp),
      (w = roomWidth),
      (h = roomHeight),
      (f = "white"),
      (idR = info.day + "#" + info.room.replace(" ", ""));
  } else if (s == "clickerSat") {
    (y = marginUp),
      (w = roomWidth),
      (h = satHeight),
      (f = "white"),
  ...
  if (s == "day" || s == "saturday") {
    rect.setAttribute("stroke", "#43cece");
  ...
  if (idR) {
    rect.setAttribute("id", idR);
  }
  if (o) {
    rect.setAttribute("style", "opacity:" + o);
  }

  return rect;
}

function addRoom(day, x, room) {
  // for creating a room
  ...
  for (var i = 0; i < 4; i++) {
    addRect(
      ...
      null,
      x,
      i * (lessonHeight * 2) + lessonHeight + marginUp,
      day
    );
  }
...
function addSaturday(semester, x, room) {
  // for creating a room
...
function makeBookings(booking) {
  ...
  var width = roomWidth;
  var x = 0;
  roomCords.forEach(function (elem2) {
    if (booking.room == elem2[0]) {
      x = elem2[1];
    }
  });
  if (para.length > 0) {
    width = width / (para.length + 1);
  ...
  if (days.indexOf(booking.day) > -1) {
    ...
    if (satHT.indexOf(booking.day) > -1) {
...
function callback(data) {
  //receiving bookings from database 
  if (!gData) {
    console.log(data);
...
function add(event) {
  //on click for making a booking

  var e = event.target;
  console.log(e.id.substring(0, e.id.indexOf("#")));
  if (
    e.id.substring(0, e.id.indexOf("#")) == "ht" ||
    e.id.substring(0, e.id.indexOf("#")) == "vt"
  ) {
    satBooking = true;
    eventDay = "lördag";
...
function update(id) {
  //on click for changing a booking
  ...
  if (booking.owner != user) {
    var confirm = window.confirm(
      "Vill du verkligen ändra den här bokningen, som har gjorts av " +
        booking.owner +
        "?"
    );
    if (confirm == false) {
...
function saveEvent() {
  //from save on the modal for making a booking
  if (user == "observer") {
    cancelModal();
    return;
  }
  ...
  if (!header) {
    document.getElementById("herjaHeader").style.display = "block";
    return;
  }
  ...
  var start =
    parseInt(startH) + Math.round((parseInt(startMin) / 60) * 1000) / 1000;
  ...
  if (satBooking) {
    //if we're making a booking for a saturday
    var json = {
      send: send,
      booking: {
        year: eventYear,
  ...
  if (document.getElementById("myModal").message) {
    //if the modal is open for updating a booking
    json = {
      id: document.getElementById("myModal").message,
      send: send,
      update: {
        header: header,
        info: info,
  ...
  var para = checkForParallell(json); 
...
function checkForParallell(booking) {
  ...
  gData.forEach(function (elem) {
    if (
      elem.year == booking.year &&
      elem.day == booking.day &&
      elem.room == booking.room &&
      booking._id != elem._id
    ) {
      if (days.indexOf(booking.day) == -1) {
        paraArr.push(elem._id);
      } else if (
        (elem.start == booking.start && elem.end == booking.end) ||
        (elem.start > booking.start && elem.start < booking.end) ||
        (elem.end < booking.end && elem.end > booking.start) ||
        (booking.start > elem.start && booking.start < elem.end) ||
        (booking.end < elem.end && booking.end > elem.start)
      ) {
        if (days.indexOf(booking.day) > -1) {
          paraArr.push(elem._id);
        }
